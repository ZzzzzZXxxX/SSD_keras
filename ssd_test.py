def test(voc_classes, checkpoints, path_test, dev="cpu",conf_num=0.6,path_result='output.csv'):
    ...
    for i, img in enumerate(images):
        ...
        top_indices = [i for i, conf in enumerate(det_conf) if conf >= conf_num]
        ...
        for i in range(top_conf.shape[0]):
            ...
            temp.append('('+str(xmin)+','+str(ymin)+','+str(abs(xmin-xmax))+','+str(abs(ymin-ymax))+')')
            temp.append(label_name)
        iofo.append(temp)
        plt.show()
    print(iofo)
    with open(path_result, 'w')as file:
        for cutWords in iofo:
